...
public final class PluginUtils {
    ...
    private static PluginManager createPluginManagerFromRootFolder(PluginConfig pluginConfig) {
        Collection<PluginDescriptor> pluginDescriptors =
                pluginConfig
                        .getPluginsPath()
                        .map(PluginUtils::getPluginDescriptorsFromPath)
                        .orElse(Collections.emptyList());
        return new DefaultPluginManager(
                pluginDescriptors, pluginConfig.getAlwaysParentFirstPatterns());
    }

    private static Collection<PluginDescriptor> getPluginDescriptorsFromPath(Path path) {
        try {
            return new DirectoryBasedPluginFinder(path).findPlugins();
        } catch (IOException e) {
